...
steps_per_interval <- aggregate(steps ~ interval, data=activity_nomissing, mean)
plot(steps_per_interval$interval, steps_per_interval$steps, main="Average steps per 5-minute interval", type="l", xlab="5-minute interval", ylab="Average number of steps")
paste("The 5-minute interval with maximum number of steps: ", max(steps_per_interval$steps))
```
...
## Finally, lets clean up the downloaded files
```{r}
if(dir.exists('activity_data/')){
  file.remove("activity_data/activity.csv")
  file.remove('activity_data/', recursive=TRUE)
}
file.remove("activitydata.zip")
```